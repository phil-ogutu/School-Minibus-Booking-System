export const getRouteFromStops = async (stops,setRouteCoordinates) => {
    try {
      ...
      if (validStops?.length < 2) {
        ...
        const fallbackRoute = validStops?.map(stop => [stop?.latitude, stop?.longitude]);
      ...
      const waypoints = validStops?.map(stop => `${stop?.longitude},${stop?.latitude}`).join(';');
      ...
      for (let i = 0; i < validStops?.length - 1; i++) {
        ...
        // Add start point
        enhancedRoute.push([start?.latitude, start?.longitude]);
        ...
        for (let j = 1; j < steps; j++) {
          const ratio = j / steps;
          const lat = start?.latitude + latDiff * ratio;
          const lng = start?.longitude + lngDiff * ratio;
      ...
      const simpleRoute = validStops?.map(stop => [stop?.latitude, stop.longitude]);
