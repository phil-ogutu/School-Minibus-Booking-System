...
from service import BusService, RouteService, UserService, DriverService
...
#     driver = DriverService.findById(data.get('driver_id'))
...
#             emit('bus_location_update_to_parent', {lat, lng}, room=f'parent_{booking.parent.id}_booking_{booking.id}')
#             parent_data = {
#                 'bus_id': bus_id,
#                 'latitude': lat,
#                 'longitude': lng,
...
#                 'timestamp': timestamp.isoformat(),
#             }
#             emit('bus_location_update_to_parent', parent_data, room=f'parent_{booking.parent_id}_booking_{booking.id}') 
#             # Parent room (search by route id) bus location to parent through trip or bus id"""
...
def joinTrackingGroup(data):
    ...
    room = f"bus{bus.id}_{bus.plate}_driver_{bus.driver.id}"
    if not bus.tracking_room:
        # create and save room
        bus.tracking_room = room
        db.session.commit()
    
    join_room(room)
    user = token_required_socketio()
    if user:
        print(user.username + ' has joined the tracking room.', room)
        emit('tracking-joined', {'user': user.username, 'room': room}, to=room)
